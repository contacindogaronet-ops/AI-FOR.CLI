...
func StartUIWithPool(cfg *config.Config, pool *core.AIClientPool, memory *core.Memory) {
	...
	for {
		...
		fmt.Println("[7] Buat Backup Workspace & History (.zip)")
		fmt.Println("[8] Restore Backup Workspace & History")
		fmt.Println("[q] Keluar")
		fmt.Println("----------------------------------------")
		fmt.Print("Pilih menu [1-8/q]: ")
		...
		case "6":
			...
			fmt.Println("========================================================")
		case "7":
			backupName := fmt.Sprintf("daemon_backup_%s.zip", time.Now().Format("20060102_150405"))
			backupPath := filepath.Join(config.GetTermuxHome(), backupName)
			fmt.Printf("\n[BACKUP] Membuat arsip state ke: %s...\n", backupPath)
			err := core.CreateDaemonBackup(backupPath)
			if err != nil {
				fmt.Printf("[Error] Gagal membuat backup: %v\n", err)
			} else {
				fmt.Printf(" [SUKSES] Backup berhasil disimpan di: %s\n", backupPath)
			}
		case "8":
			fmt.Print("Masukkan path file .zip backup yang ingin direstore: ")
			if scanner.Scan() {
				zipTarget := strings.TrimSpace(scanner.Text())
				if zipTarget != "" {
					fmt.Println("\n[RESTORE] Memulihkan file konfigurasi dan riwayat memori...")
					err := core.RestoreDaemonBackup(zipTarget)
					if err != nil {
						fmt.Printf("[Error] Gagal merestore backup: %v\n", err)
					} else {
						fmt.Println(" [SUKSES] State & History berhasil dipulihkan! Silakan restart daemon.")
					}
				}
			}
		case "q":
